...
BOOL StatusBarWindowSetText( LPCTSTR lpszText )
{
	// Set status bar window text
	return SendMessage( g_hWndStatusBar, SB_SETTEXT, ( WPARAM )NULL, ( LPARAM )lpszText );

} // End of function StatusBarWindowSetText

int StatusBarWindowSize()
{
	int nResult;
	...
	// Calculate status bar window height
	nResult = ( rcStatus.bottom - rcStatus.top );
